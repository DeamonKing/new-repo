...
function updatePipelineDropdowns() {
    ...
    dropdowns.forEach(dropdown => {
        const selectedPipeValue = dropdown.value;
        ...
        // Disable options that are already selected in other dropdown menus
        Object.values(selectedPipelines).forEach(selectedPipe => {
            dropdown.querySelectorAll('option').forEach(option => {
                if (option.value === selectedPipe && option.value !== selectedPipeValue) {
                    option.disabled = true; // Disable already selected pipelines
                }
            });
        });
    });
